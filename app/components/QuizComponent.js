...
const QuizComponent = () => {
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.results && data.results.length > 0) {
                    ...
                    setScoreSaved(false);
                    shuffleAnswers(data.results[0]); // Shuffle answers for the first question
                } else {
                    setLoading(false);
    ...
    // Function to shuffle answers for the current question
    const shuffleAnswers = (question) => {
        if (question && Array.isArray(question.incorrect_answers) && typeof question.correct_answer === 'string') {
    ...
    // Function to decode HTML entities in the quiz questions and answers
    ...
    // useEffect to save the score when the quiz is completed
    useEffect(() => {
    ...
    // Render the initial quiz setup screen
    if (!quizStarted) {
        return (
    ...
    // Render the quiz completed screen
    if (quizCompleted) {
        return (
            <div className="min-h-96 md:min-h-screen flex flex-col justify-center items-center bg-gray-100 p-4">
    ...
    // Render message if no questions are available
    ...
    return (
        <div className="min-h-screen flex flex-col justify-center items-center bg-gray-100 p-4">
            <p className="mb-4 text-lg">Signed in as {session.user.name}</p>
            <p className="mb-4 text-lg font-bold">Question {currentQuestionIndex + 1} of {questions.length}</p> {/* Display the current question number */}
